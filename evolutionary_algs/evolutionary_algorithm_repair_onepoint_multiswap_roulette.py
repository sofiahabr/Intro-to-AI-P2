...
def run(test_file): 
    ...
    def mutate(sol, num_replacements=2):
        sol = sol.copy()
        
        num_replacements = min(num_replacements, m)
        
        # Pick random positions to replace
        positions = random.sample(range(m), num_replacements)
        
        for pos in positions:
            # Pick a random site from candidate pool that's not already in solution
            new_site = random.randint(0, c - 1)  # or (1, c) depending on your indexing
            
            while new_site in sol:
                new_site = random.randint(0, c - 1)
